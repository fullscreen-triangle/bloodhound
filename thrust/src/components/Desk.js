...
export function Computers(props) {
    ...
    // Store original positions once the group mounts
    const originals = useRef([])
    useEffect(() => {
        if (!groupRef.current) return
        originals.current = groupRef.current.children.map((c) => c.position.clone())
    }, [])

    // Gentle breathing / drift animation using sine waves — no external dependency
    useFrame((state) => {
        if (!groupRef.current) return
        ...
        groupRef.current.children.forEach((child, i) => {
            ...
            const wave = Math.sin(t * 0.3 + i * 0.7) * 0.18
            child.position.set(orig.x + dir.x * wave, orig.y + dir.y * wave, orig.z + dir.z * wave)
        })
    })
    return (
        <group {...props} dispose={null}  ref={groupRef}>
            ...
            <ScreenVideo videoSrc="/gifs/assisted-takeoff.gif"
                         frame="Object_206"
                         panel="Object_207"
                         position={[0.27, 1.53, -2.61]}
            />
            <ScreenVideo videoSrc="/gifs/wireframe-cheetah.gif" frame="Object_209" panel="Object_210" y={5} position={[-1.43, 2.5, -1.8]} rotation={[0, 1, 0]} />
            <ScreenText invert frame="Object_212" panel="Object_213" x={-5} y={5} position={[-2.73, 0.63, -0.52]} rotation={[0, 1.09, 0]} />
            <ScreenVideo videoSrc="/gifs/rugby-paint.gif" frame="Object_215" panel="Object_216" position={[1.84, 0.38, -1.77]} rotation={[0, -Math.PI / 9, 0]} />
            ...
            <ScreenVideo videoSrc='/gifs/7mmpbg.gif' frame="Object_227" panel="Object_228" position={[0.96, 4.28, -4.2]} rotation={[0, -0.65, 0]} />
            <ScreenVideo videoSrc='/gifs/pen.gif' frame="Object_230" panel="Object_231" position={[4.68, 4.29, -1.56]} rotation={[0, -Math.PI / 3, 0]} />
