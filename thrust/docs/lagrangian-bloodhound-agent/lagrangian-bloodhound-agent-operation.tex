...
\newcommand{\hbar_A}{\hbar_{\mathcal{A}}}
...
\begin{tabular}{lll}
\toprule
Regime & $R$ range & Qualitative description \\
\midrule
Turbulent & $R < 0.3$ & Incoherent; stochastic exploration dominates \\
Aperture-Dominated & $0.3 \le R < 0.5$ & Constraints govern motion; $\Aper$ is active \\
Hierarchical Cascade & $0.5 \le R < 0.8$ & Structured descent through memory tiers \\
Coherent & $0.8 \le R < 0.95$ & Directed, near-deterministic navigation \\
Phase-Locked & $R \ge 0.95$ & Partition extinction; zero remaining friction \\
\bottomrule
\end{tabular}
\end{center}
\end{definition}
...
\begin{equation}
  \beta_{\mathrm{composite}} = \sum_{i=1}^n \beta_i - \frac{1}{\Sigma}\sum_{i < j}\beta_i\beta_j + \cdots
...
\end{equation}
Dividing by $\Sigma$ and using $\beta_i = \abs{D_i}/\Sigma$ yields \eqref{eq:composite_floor}.  The lower bound $\beta_{\mathrm{composite}} \ge \max_i\beta_i$ follows from monotonicity of the join: any join is finer than each factor.
\end{proof}
...
\label{thm:critical}
For a homogeneous ensemble ($K_{ij} = K$ for all $i\ne j$) with natural frequency distribution $g(\omega)$ having standard deviation $\sigma_\omega$, synchronisation ($R_\mathrm{ens} > 0$) occurs if and only if the coupling strength $K$ exceeds the critical value \citep{kuramoto1984,strogatz2000,acebron2005}:
\begin{equation}
...
\begin{proof}
Standard Kuramoto analysis \citep{kuramoto1984,acebron2005}.  The self-consistency equation $R_\mathrm{ens} = R_\mathrm{ens}\int_{-\pi/2}^{\pi/2}\cos^2(\theta)\,g(KR_\mathrm{ens}\sin\theta)\,d\theta$ has the trivial solution $R_\mathrm{ens} = 0$ for all $K$, and a non-trivial solution for $K > K_c = 2/(\pi g(0)) = 2\sigma_\omega/\pi$ (using $g(0) = 1/\sigma_\omega$ for a Lorentzian distribution).
\end{proof}
...
\toprule
Regime & $R_\mathrm{ens}$ & Coordination property \\
\midrule
Turbulent & $< 0.3$ & No coherent ensemble dynamics; agents act independently \\
Aperture-Dominated & $[0.3, 0.5)$ & Structural constraints dominate; partial phase alignment \\
Hierarchical Cascade & $[0.5, 0.8)$ & Memory-tier-structured coordination; cascade routing active \\
Coherent & $[0.8, 0.95)$ & Near-deterministic joint navigation; shared aperture accessible \\
Phase-Locked & $\ge 0.95$ & Partition extinction; zero coordination friction (analogous to Cooper-pair formation \citep{cooper1956,bardeen1957}) \\
\bottomrule
\end{tabular}
\end{center}
\end{definition}
...
\toprule
Code & Name & Diagnostic signature \\
\midrule
P1 & Construction-Deficient & $A_\mathrm{con} \ll A_\mathrm{con}^0$; agent fails (I1): cannot generate novel cells \\
P2 & Hyper-Constructive & $A_\mathrm{con} \gg A_\mathrm{act}$; agent builds cells without acting; $I \to \infty$ \\
P3 & Construction-Deprived & $T_\mathrm{con} \ll T_\mathrm{ref}$; insufficient COMPILE time; condition (I4) unmet \\
P4 & Perceptually-Decoupled & $\kappa_\mathcal{A} \approx 0$; no floor reduction on novel cells; condition (I3) fails \\
P5 & Action-Cycle Disrupted & EXECUTE phase terminates prematurely; $\sigma_Y$ never large \\
P6 & Construction-Cycle Disrupted & COMPILE phase terminates prematurely; $\sigma_K$ never large \\
\bottomrule
\end{tabular}
\end{center}
